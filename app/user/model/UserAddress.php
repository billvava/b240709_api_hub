...
class UserAddress extends Model
{
    ...
    //默认地址
    public function get_def_address($user_id,$is_master=0)
    {   
        if($is_master==1){
            $info = $this->removeOption()->where(array('master_id' => $user_id))->order('is_default desc')->find();
        }else{
            $info = $this->removeOption()->where(array('user_id' => $user_id))->order('is_default desc')->find();
        }
    ...
    public function get_info($user_id, $id,$is_master=0)
    {
        if($is_master==1){
            $info = $this->removeOption()->where(array('master_id' => $user_id, 'id' => $id))->find();
        }else{
            $info = $this->removeOption()->where(array('user_id' => $user_id, 'id' => $id))->find();
        }
     ...
     * @param type $options
     */
    public function set_default($is_default, $id, $user_id)
    {
    ...
    public function getList($in)
    {
        ...
        $status_arr = array('user_id', 'is_default', 'province', 'city', 'country');
        foreach ($in as $k => $v) {
    ...
    public function rules()
    {
        return array(
            ...
            array('user_id', 'require', '用户ID不能是空！', 0),
            array('user_id', 'checkIsInt', '用户ID必须是整数', 0, 'callback'),
