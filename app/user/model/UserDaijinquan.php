 ...
 * @mixin think\Model
 */
class UserDaijinquan extends Model {


    protected $name='user_daijinquan';
    ...
    public  function getAll($where = array(),$cache=true){
       ...
       $name = "user_daijinquan_1721377237_{$pre}";
       $data = cache($name);
       if(!$data  || !$cache){
           $order = "id desc";
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "user_daijinquan_info_1721377237_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function clear($pk = ''){
        $name = "user_daijinquan_1721377237";
        cache($name,null);
        if ($pk) {
            $name = "user_daijinquan_info_1721377237_{$pk}";
            cache($name, null);
