...
class Address {
    ...
    public function getShopInfo()
    {
        ...
        $state_array = ['待审核','已通过','未通过'];
        $this->data['info'] = Db::name('shop')->where('uid',$this->uinfo['id'])->find();
        $oh=new O();

        $array = ['2'=>'市级代理','3'=>'县级代理','4'=>'店家'];
        if ($this->data['info']) {
            $this->data['info']['province_name']=$oh->getAreas($this->data['info']['province']);
            $this->data['info']['city_name']=$oh->getAreas($this->data['info']['city']);
            $this->data['info']['country_name']=$oh->getAreas($this->data['info']['country']);
            $area_name = $this->data['info']['province_name'].'-'.$this->data['info']['city_name'].'-'.$this->data['info']['country_name'];
            $this->data['info']['address_text'] =$area_name;
            $this->data['info']['agent_name'] = $array[$this->data['info']['type']]??'';
            $this->data['info']['state_text'] = $state_array[$this->data['info']['state']]??'';
        }
    ...
    public function item()
    {
        ...
        $this->in['reference_address'] = $this->in['address_text'];
    ...
    public function addShop()
    {
        ...
        if(!verify_mobile($this->in['tel'])){
            return array('status' => 0, 'info' => '手机格式有误');
        }
        $this->in['uid'] = $this->uinfo['id'];
        $this->in['create_time'] = time();

        if($this->in['id']){
            $this->in['state'] = 0;
            if($this->in['type'] !=4){
                $data = Shop::where('type',$this->in['type']) -> find();
                if($data['id'] != $this->in['id']){
                    return array('status' => 0, 'info' => '县级代理或市级代理只能申请一个');
                }
            }
            $res = Shop::where('id',$this->in['id']) -> update($this->in);
        }else{
            if($this->in['type'] !=4){
                if(Shop::where('type',$this->in['type']) -> count()){
                    return array('status' => 0, 'info' => '县级代理或市级代理只能申请一个');
                }
            }
            $res = Shop::insertGetId($this->in);
        }
        if($res){
    ...
    public function getShop(){
        $array = ['待审核','已通过','未通过'];
        $uid = $this->uinfo['id'];
        $res = Shop::where('uid',$uid) -> find();
        $res['state'] = $array[$res['state']]??'';
