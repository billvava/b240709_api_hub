...
class Atn {
    ...
    public function atn() {
        ...
        $is_atn = $this->goodsLogic->isAtn($this->in['goods_id'], $this->uinfo['id']);
        $f = false;
        $info = '已取消';
        if (!$is_atn) {
            $this->goodsLogic->addAtn($this->in['goods_id'], $this->uinfo['id']);
            $f = true;
            $info = '已收藏';
        } else {
            $this->goodsLogic->delAtn($this->in['goods_id'], $this->uinfo['id']);
        }
