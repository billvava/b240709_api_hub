...
class Cart {
    ...
    public function config($map) {
        ...
        $this->user_id = $this->uinfo['id'];
        $this->model = Db::name('mall_cart');
        $info = $this->model->where(array('user_id' => $this->user_id))->find();
        if (!$info) {
            $this->model->insert(array('user_id' => $this->user_id));
        } else {
    ...
    public function up_data($item = null) {
        ...
        $this->model->where(array('user_id' => $this->user_id))->update(array(
    ...
    public function add() {
        ...
        return array('status' => 0, 'info' => '此商品暂不支持加入购物车');
        ...
        $GoodsModel = new \app\mall\model\Goods();
    ...
    public function clear() {
        $this->items = array();
        $this->model->where(array('user_id' => $this->user_id))->delete();
        return array('status' => 1);
    }
