...
class Groups
{
    ...
    public function item()
    {
        ...
        $logic = new \app\mallapi\logic\Goods();
        $logic->config(array(
            'in' => $this->in,
            'uinfo' => $this->uinfo,
            'data' => $this->data
    ...
    public function pt_order()
    {
        ...
        if ($this->in['ordernum']) {
            $info = Db::name('pt_order')->where(array('user_id' => $this->uinfo['id'], 'ordernum' => $this->in['ordernum']))->find();
            ...
            if ($info) {
                ...
                if ($info['type'] == 1) {
                    ...
                    $user_ids = Db::name('pt_order')->where(array('p_ordernum' => $info['p_ordernum']))->order("time asc")->column('user_id');
                    $user_data = array();
                    if ($user_ids) {
