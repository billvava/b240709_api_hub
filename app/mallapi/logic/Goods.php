...
class Goods
{
    ...
    public function __construct()
    {
        ...
        $this->mastermodel = new \app\common\model\SuoMaster();
    }
    ...
    public function index()
    {
        ...
        if ($this->in['load_other'] == 1) {
            $this->data['category_list'] = Db::name('mall_goods_category')->where(array('is_show' => 1, 'pid' => 1))->order("sort desc")->cache(true)->select()->toArray();
    ...
    public function group()
    {
        ...
        if ($this->in['group_id']) {
            ...
            $MsGoods = new \app\mall\model\PtGoods();
    ...
    public function item()
    {
        ...
        foreach ($this->data['com_list'] as &$v) {
            ...
            if($v['master_id']){
                $uu = $this->mastermodel->getUserInfo($v['master_id']);
            }else{
                $uu = $this->model->getUserInfo($v['user_id']);
            }
        ...
        $this->data['is_atn'] = (new \app\mall\logic\Goods())->isAtn($this->in['goods_id'], $this->uinfo['id']);
    ...
    public function get_coupon()
    {
        ...
        $cs = Db::name('mall_coupon')->where(array('user_id' => $this->uinfo['id']))->column('tpl_id');
        $num = 0;
    ...
    public function get_goods_comment()
    {
        ...
        $Comment = new \app\mall\model\Comment();
    ...
    public function load_list()
    {
        ...
        $map = [];
        $map[] = ['english','<>','manghe'];
        $category_ids = Db::name('mall_goods_category') -> where($map)-> column('category_id');

        $map1 = [];
        $map1[] = ['category_id','in',$category_ids];
        $res = $this->goodsModel->get_data($in,$map1);
    ...
    private function create_goods_share($user_id, $ginfo)
    {
        $temp_ewm_path = C('temp_ewm_path');
        $path = $temp_ewm_path . "{$ginfo['goods_id']}/";
        if (!is_dir($path)) {
        ...
        if (!file_exists($file) || 1) {
            ...
            $font = './static/admin/font/aliheiti.ttf';
            if ($is_win) {
                $font = getcwd() . '\static\admin\font\aliheiti.ttf';
            }
            imagefttext($BaseImg, 22, 0, 20, 550, $color, $font, $one);
            if ($two) {
                imagefttext($BaseImg, 22, 0, 20, 580, $color, $font, $two);
            }
            //写文字
            $color = imagecolorallocate($BaseImg, 243, 34, 36);
            $ginfo['min_price'] = ceil($ginfo['min_price']);
    ...
    private function get_cxx_ewm($user_id)
    {
        return array('status' => 1, 'img' => file_get_contents("./img/empHeader.jpg"));
//        $url = "/pages/goods/item/item?goods_id={$goods_id}&pid={$user_id}";
