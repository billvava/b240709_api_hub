...
//            'money' => '余额',
        ...
        $this->data['total'] = $this->data['finfo']['balance'];
    ...
    {
        $page = $this->in['page'] ?: 1;
        $page_size = $this->in['page_size'] ?: 10;
        
        $map = [];
        $map[] = ['master_id','=',$this->uinfo['id']];
        $time = $this->in['time'] ?: date('Y-m');

        $list = Db::name('user_cashout_apply')->where($map) -> whereMonth('time',$time) ->orderRaw("id desc")-> paginate(['page' => $page, 'list_rows' => $page_size]);
        foreach($list as $key => $value){
          // 状态|0=未处理,1=已发放,2=退回,3=待核实
          $array = ['未处理','已发放','退回','待核实'];
          $value['status_text'] = $array[$value['status']] ?? '';
          $list[$key] = $value;
        }
        $data = [];
        $total_money = Db::name('user_cashout_apply')->where($map) -> where('status',1) -> sum('money');
