...
class HelpMsg extends Common{
    ...
    public function __construct(App $app)
    {
        ...
        $this->model = new \app\com\model\HelpMsg();
    ...
    */
    ...
    private function item() {
        ...
        if ($this->request->isPost()) {
            ...
            if ($in[$this->pk]) {
                $info = $this->model->where($this->pk,$in[$this->pk])->find();
                if($in['status'] == 1 && $info['status'] == 0&& $info['order_id'] ){
                    Db::name('suo_order')->where([
                        ['id','=',$info['order_id']]
                    ])->save([
                        'tousu_status'=>2
                    ]);
