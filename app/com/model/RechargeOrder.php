...
class RechargeOrder extends Model {
    ...
    public  function getAll($where = array()){
       ...
       $name = "recharge_order_1616730839_{$pre}";
       $data = cache($name);
       if(!$data){
           $order = "id desc";
    ...
    public  function getInfo($pk) {
        ...
        $name = "recharge_order_info_1616730839_{$pk}";
        $data = cache($name);
        if (!$data) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public function getLan($field=''){
        $lans = array('is_pay' => array('1'=>'是','0'=>'否',),'is_feek' => array('1'=>'是','0'=>'否',),);
        if($field==''){
            return $lans;
        }
        return $lans[$field];
    ...
    public  function clear($pk = ''){
        $name = "recharge_order_1616730839";
        cache($name,null);
        if ($pk) {
            $name = "recharge_order_info_1616730839_{$pk}";
            cache($name, null);
...
'is_feek'=>'退款状态',
    ...
    public  function rules() {
        return [
            'rule'=>[
            'ordernum|订单号'=>["max"=>255,],
'user_id|用户ID'=>["integer",],
'create_time|创建时间'=>[],
'goods_total|订单金额'=>["float",],
'total|金额'=>["float",],
'is_pay|支付状态'=>["integer",],
'tra_no|流水号'=>["max"=>255,],
'pay_time|支付时间'=>[],
'pay_type|支付方式'=>["integer",],
'is_feek|退款状态'=>["integer",],
