...
class Order extends Model
{
    ...
    //检测订单的拥有者
    public function checkOrderOwner($order_id, $master_id)
    {
        if (!$order_id || !$master_id) {
            return false;
        }
        $master_id2 = $this->where(array('order_id' => $order_id))->value('master_id');
        if ($master_id != $master_id2) {
            return false;
        }
        return true;
    }
...
//        }
    ...
    public function getOrderItemOrdernum($ordernum, $type, $master_id = null)
    {
        ...
        if ($type == 'home' && $info['master_id'] != $master_id) {
    ...
    //订单详情
    public function getOrderItem($order_id, $type, $master_id = null)
    {
        if (!in_array($type, array('home', 'admin'))) {
            return array('status' => 0, 'info' => '缺少参数type');
        }
    ...
    public function get_data($in = array())
    {
        ...
        $status_arr = array('delivery_type', 'p_ordernum', 'comment_status', 'master_id', 'status', 'type', 'is_u_del', 'is_a_del', 'after_status', 'ip', 'order_id', 'pay_type', 'shop_id', 'pay_status');
        foreach ($in as $k => $v) {
...
//        sqlListen();
    ...
    //用户统计
    public function getUserAnay($master_id, $cache = false)
    {
        $where = array('master_id' => $master_id, 'pay_status' => 1);
    ...
    //获取分销订单
    public function getSaleOrder($master_id, $page = 1, $date = '')
    {
        if ($page > 0) {
            $_GET['p'] = $page;
        }
        ...
        $data['page'] = $Page->show();
        $data['count'] = $count;
        $data['list'] = Db::name('mall_order')->alias('o')->leftJoin('user u', 'o.master_id=u.id')->where($where)->field(
        ...
        foreach ($data['list'] as &$v) {
            ...
            $v['bro'] = 0;
            if ($rs[$v['order_id']]['pid1'] == $master_id) {
                $v['bro'] = $rs[$v['order_id']]['bro1'];
                $v['lev'] = '一级';
            } else if ($rs[$v['order_id']]['pid2'] == $master_id) {
                $v['bro'] = $rs[$v['order_id']]['bro2'];
                $v['lev'] = '二级';
            } else if ($rs[$v['order_id']]['pid3'] == $master_id) {
                $v['bro'] = $rs[$v['order_id']]['bro3'];
                $v['lev'] = '三级';
            }
//            $v['user_info'] = $userModel->getUserInfo($v['master_id']);
        }
        return $data;
    }

    public function search($str, $master_id)
    {
...
//                'imageUrl'
        ...
        if ($order_info['delivery_type'] == 1 && $order_info['status'] == 2 && $order_info['delivery_status'] == 1) {
    ...
    //奖励发放（推荐奖）
    public  function recommendedAward($uid='',$re_id='',$money=0,$ordernum=''){
        if($uid && $money > 0){
            $this->userModel ->handleUser('money', $re_id, $money, 1, array('cate' => 12, 'ordernum' => $ordernum));
        }
    }

    //极差奖
    public function jichajiang($uid,$re_id,$money=0,$ordernum=''){
        $user_data = $this->userModel ->where('id',$uid) -> find();
        $main_account_id = $this->userModel ->where('is_main',1) -> value('id'); //主营账号
        $shop_id1 = $user_data['shop_id'];//店长id
        $shop_id = Db::name('shop') -> where('id',$shop_id1) -> value('uid');
        $country = $user_data['country'];//县id
        $city = $user_data['city'];//市id
        $map = [];
        $map[] = ['country','=',$country];
        $map[] = ['type','=',3];
        $map[] = ['state','=',1];
        $country_uid = Db::name('shop') -> where($map) -> value('uid');//县级代理

        $map1 = [];
        $map1[] = ['city','=',$city];
        $map1[] = ['type','=',2];
        $map1[] = ['state','=',1];
        $city_uid = Db::name('shop') -> where($map1) -> value('uid');//市级代理

        //$role =  $this->userModel ->where('id',$re_id) -> value('role');
        $array_bonus = explode('|',C('proportion'));
        $shop_bonus = $money*$array_bonus[3]/100;//店长奖励
        $country_bonus = $money*$array_bonus[2]/100;//县级奖励
        $city_bonus = $money*$array_bonus[1]/100;//市级奖励
        $main_account_bonus = $money*$array_bonus[0]/100;//主营账号奖励

        if($shop_id){
            $this->userModel ->handleUser('money', $shop_id, $shop_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
        }
        if($country_uid && $city_uid){
            $this->userModel ->handleUser('money', $country_uid, $country_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));

            $this->userModel ->handleUser('money', $city_uid, $city_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));

            $this->userModel ->handleUser('money', $main_account_id, $main_account_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
        }


        if(!$country_uid && $city_uid){ //假如有市级代理没有县级代理
            $city_bonus = $city_bonus +  $country_bonus;
            $this->userModel ->handleUser('money', $city_uid, $city_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
            $this->userModel ->handleUser('money', $main_account_id, $main_account_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
        ...
        if($country_uid && !$city_uid){ //假如有县级代理和没有市级代理
            $this->userModel ->handleUser('money', $country_uid, $country_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
            $main_account_bonus =  $main_account_bonus + $city_bonus;
            $this->userModel ->handleUser('money', $main_account_id, $main_account_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
           
        }

        if(!$city_uid && !$country_uid){ //假如没有县级代理和没有市级代理
            $main_account_bonus =  $main_account_bonus + $country_bonus + $city_bonus;
            $this->userModel ->handleUser('money', $main_account_id, $main_account_bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
            
        }

        // if($bonus>0){
        //     $this->userModel ->handleUser('money', $re_id, $bonus, 1, array('cate' => 13,'ordernum' => $ordernum));
        // }

    }

    //代理推荐奖励
    public  function agentBonus($re_id){
        $role =  $this->userModel ->where('id',$re_id) -> value('role');
        $array_bonus = C('agent_bonus');
        $bonus = $array_bonus[$role-2];
        if($bonus>0){
            $this->userModel ->handleUser('money', $re_id, $bonus, 1, array('cate' => 14));
        }
    }

    //会员升级 uid//推荐人id
    public  function upgrade($uid,$pid,$money=0,$ordernum=''){
        if($money==499){ //购买499礼包成为普通会员
            $rank = User::where('id',$pid)->value('rank');//推荐的人等级
            
            $this->userModel ->where('id',$uid)->update(['rank' => 1]);
            
            $map[] = ['pid','=',$pid];
            $map[] = ['rank','>=',1];
            $count =  $this->userModel ->where($map)->count();
            if($count >=2 && $rank < 2){ //直推荐二个499后，得到推荐奖499，成为VIP会员，再直推荐更多的499也不能再得推荐奖
                $this->userModel -> where('id',$pid)->update(['rank' => 2]);  //vip会员
                $this -> recommendedAward($uid,$pid,$money,$ordernum);
            }


            //直推荐第三个以上的499
            if($count >=3){
                $gift_pack = C('gift_pack');
                if($gift_pack>0){
                    $this->userModel ->handleUser('bro', $pid, $gift_pack, 1, array('cate' => 12));
                }
            }
        }
    }
