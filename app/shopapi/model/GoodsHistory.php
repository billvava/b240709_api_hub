...
class GoodsHistory extends Model {
    ...
    public function addLog($goods_id, $master_id, $type = 1) {
        if (!$goods_id || !$master_id) {
            return false;
        }
        $this->where(array('master_id' => $master_id, 'goods_id' => $goods_id, 'type' => $type))->delete();
        $this->insert(array('master_id' => $master_id, 'goods_id' => $goods_id, 'type' => $type, 'time' => date('Y-m-d H:i:s')));
        return true;
    }

    public function getUserList($master_id, $page = 1) {
        $where = array(
    ...
    public function getCount($master_id){
        $where = array(
            'a.master_id' => $master_id,
            'b.status' => 1,
        );
