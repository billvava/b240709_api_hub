...
//                p($e->getMessage());
    ...
    public function pay_success($order_info)
    {
        ...
        $DB_PREFIX = DB_PREFIX;
        $t = date('Y-m-d H:i:s');
        $sql = "update {$DB_PREFIX}user_ext set mall_total= mall_total + {$order_info['total']},mall_new_time='{$t}',mall_order_num=mall_order_num+1 where master_id={$order_info['master_id']}";
        tool()->classs('Mysql_query');
        $Mysql_query = new \Mysql_query();
        $Mysql_query->query($sql);
        $sql = "update {$DB_PREFIX}user_ext set mall_order_avg=(mall_total/mall_order_num)  where master_id={$order_info['master_id']}";
        $Mysql_query->query($sql);
    ...
    public function close($order_id, $map)
    {
        ...
        if ($map['type'] == 'home' && $order_info['master_id'] != $map['master_id']) {
        ...
        $userModel = new User();
//        if ($order_info['money']) {
//            $userModel->handleUser('money', $order_info['master_id'], $order_info['money'], 1, array('cate' => 3, 'ordernum' => $order_info['ordernum']));
//        }
        if ($order_info['dot']) {
            $userModel->handleUser('dot', $order_info['master_id'], $order_info['dot'], 1, array('cate' => 3, 'ordernum' => $order_info['ordernum']));
        }
        if (C('weixin_msg') == 1) {
            $commonWx = new Weixin();
            $msg = "您的订单【{$order_info['ordernum']}】已经关闭。";
            $openid = (new User())->getOpenid($order_info['master_id']);
    ...
    public function up_total($order_id, $total)
    {
        ...
        Db::name('user_bro')->where(array('ordernum' => $order_info['ordernum'], 'master_id' => $order_info['master_id']))->save(array('ordernum' => $newOrdernum));
        Db::name('user_dot')->where(array('ordernum' => $order_info['ordernum'], 'master_id' => $order_info['master_id']))->save(array('ordernum' => $newOrdernum));
        Db::name('user_money')->where(array('ordernum' => $order_info['ordernum'], 'master_id' => $order_info['master_id']))->save(array('ordernum' => $newOrdernum));
    ...
    public function refund($order_id, $refund_money)
    {
        ...
        if ($order_info['pay_type'] == 5) {
            ...
            $userModel->handleUser('money', $order_info['master_id'], $refund_money, 1, array('cate' => 3, 'ordernum' => $order_info['ordernum']));
            $flag = 1;
        }
    ...
    public function send_dot($order_info)
    {
        if (bccomp($order_info['goods_total'], 0) == 1 && C('dot_mall') == 1) {
            ...
            if ($dot > 0) {
                (new User())->handleUser('dot', $order_info['master_id'], $dot, 1, array('cate' => 4, 'ordernum' => $order_info['ordernum']));
            }
        }
    }
