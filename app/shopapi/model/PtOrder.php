...
{
    public function getTeamList($group_id, $master_id, $ordernum = '')
    {
        $list = [];
        $userModel = new User();
        ...
        if ($ordernum) {
            $data = $this->removeOption()->where([
                ['p_ordernum', '=', $ordernum]
            ])->field('master_id,group_num,type')->order('type asc')->select()->toArray();
            if ($data) {
                ...
                for ($i = 0; $i < $pt_num; $i++) {
                    if ($data[$i]) {
                        $item = $data[$i];
                        $uinfo = $userModel->getUserInfo($item['master_id']);
                        $list[] = [
            ...
            for ($i = 1; $i <= $pt_num; $i++) {
                $uinfo = $userModel->getUserInfo($master_id);
                if ($i == 1) {
                    $list[] = [
                        'nickname' => $uinfo['nickname'],
                        'headimgurl' => get_img_url($uinfo['headimgurl']),
    ...
    public function getCurrentList($group_id,$p_ordernum='')
    {
        ...
        $userModel = new User();
        foreach ($data as &$v) {
            $uinfo = $userModel->getUserInfo($v['master_id']);
    ...
    public function get_data($in = array())
    {
        ...
        $status_arr = array('master_id', 'shop_id', 'goods_id', 'type', 'status');
        foreach ($in as $k => $v) {
