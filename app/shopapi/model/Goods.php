...
class Goods extends Model {
    ...
    //获取商品列表
    public static function get_data($in = array()) {
        $where = array();
        ...
        $is = array('goods_id', 'is_new', 'is_recommend', 'brand_id', 'shop_id', 'master_id','master_id');
        foreach ($is as $v) {
            if ($in[$v] != '') {
                $where[] = [$v, '=', $in[$v]];
            }
        ...
        if ($in['page_type']) {
            $count = Db::name('mall_goods')->alias('a')->where($where)->count();
    ...
    public function getInfo($goods_id, $get_ext = 0, $is_cache = false) {
        ...
        if ($get_ext == 1) {
            ...
            $goodsInfo['save_money'] = $goodsInfo['min_price'] - $goodsInfo['min_price2'];//立省金额 用户端的价格减去门店端的价格
...
//        $goodsInfo['url'] = url('Mall/Goods/item', array('goods_id' => $goods_id));
        ...
        $goodsInfo['min_price_old'] = $goodsInfo['min_price'];

        $goodsInfo['min_price'] = $goodsInfo['min_price2'];
