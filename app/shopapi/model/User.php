...
class User extends \app\common\model\User {
    ...
    public function comment_list($map = array()) {
        ...
        if ($style == 'front') {
            $where[] = ['a.master_id', '=', $map['master_id']];
            $where[] = ['a.status', '=', 1];
        }
        if ($map['goods_id']) {
    ...
    public function coupon($in = array(), $page_type = 'home') {
        ...
        //用户名查询
        if ($in['master_id']) {
            $where['a.master_id'] = $in['master_id'];
        }
    ...
    //佣金提现日志
    public function put_bro_log($master_id, $page = 1) {
        $user_money_put = Db::name('user_bro_put');
