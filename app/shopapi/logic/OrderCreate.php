...
use app\admin\model\SuoShop as Shop;
use app\shoapi\logic\Delivery;
use app\shoapi\model\MallBroRule;
...
class OrderCreate
{
    ...
    {
        $this->goodsLogicModel = new \app\shopapi\logic\Goods();
    ...
    public function create_order()
    {
        ...
        $this->check_return($this->handle_goods());
        $this->get_source();
    ...
    public function sub()
    {
        ...
        $oid = $this->orderModel->where(array('master_id' => $this->uinfo['id']))->value('order_id');
        if (!$oid) {
            $this->clear['is_new'] = 1;
        }
...
//        $this->Discount->handle('act', 'sub_after');
    ...
    private function handle_goods()
    {
        ...
        foreach ($this->data['goods_list'] as $k => &$v) {
            ...
            $v['info'] = $this->goodsModel->getInfo($v['goods_id'], true, true);
            if ($v['info']['status'] != 1) {
