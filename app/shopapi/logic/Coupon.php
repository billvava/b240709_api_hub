...
class Coupon {
    ...
    public function __construct() {
        $this->model = new \app\shopapi\model\MallCoupon();
    }
    ...
    public function draw() {
        ...
        if ($info['type'] == 2) {
            ...
            $is = $this->model->where(array('master_id' => $this->uinfo['id'], 'tpl_id' => $info['id']))->value('id');
            if ($is) {
    ...
    public function check_draw() {
        $userModel = new app\common\model\SuoMaster();
        //新人优惠券
        $get_coupon = $userModel->getExtField($this->uinfo['id'], 'get_coupon');
        $mall_coupon_tpl = Db::name('mall_coupon_tpl');
        if ($get_coupon == 0) {
        ...
        if (!$this->data['coupon_info']) {
            ...
            $is = $this->model->where(array('master_id' => $this->uinfo['id'], 'tpl_id' => $this->data['coupon_info']['id']))->value('id');
            if ($is) {
    ...
    public function my_coupon() {
        ...
        $cs = $this->model->where(array('master_id' => $this->uinfo['id']))->column('tpl_id');
