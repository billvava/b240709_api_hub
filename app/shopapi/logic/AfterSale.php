...
class AfterSale
{
    ...
    {
        $this->model = new \app\shopapi\model\AfterSale();
    ...
    {
        $map = array('master_id' => $this->uinfo['id'],
            'no_cache' => 1,
            'pay_status' => 1,
            'is_u_del' => 0,
            'get_status' => 1,
    ...
    public function index_handing()
    {
        ...
        $where = [['a.master_id', '=', $this->uinfo['id']]];
        if ($this->in['flag'] == 1) {
    ...
    public function apply_after_get()
    {
        ...
        $content = new \app\shopapi\model\Content();
    ...
    public function apply_after_sub()
    {
        ...
        $log = array(
            ...
            'content' => \app\shopapi\model\AfterSale::USER_APPLY_REFUND,
            'create_time' => date('Y-m-d H:i:s')
        );
        //记录日志
    ...
    public function cacel()
    {
        ...
        if ($info['master_id'] != $this->uinfo['id']) {
        ...
        $log = array(
            ...
            'after_id' => $info['id'],
            'handle_id' => $this->uinfo['id'],
            'content' => \app\shopapi\model\AfterSale::USER_CANCEL_REFUND,
            'create_time' => date('Y-m-d H:i:s')
        );
        //记录日志
        Db::name('mall_after_log')->insert($log);
    ...
    public function send()
    {
        ...
        $log = array(
            ...
            'after_id' => $info['id'],
            'handle_id' => $this->uinfo['id'],
            'content' => \app\shopapi\model\AfterSale::USER_SEND_EXPRESS,
            'create_time' => date('Y-m-d H:i:s')
        );
        //记录日志
        Db::name('mall_after_log')->insert($log);
