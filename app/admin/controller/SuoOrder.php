...
class SuoOrder extends BCOM{
    ...
    public function __construct(App $app)
    {
        ...
        $this->model = new \app\admin\model\SuoOrder();
...
//        $where[] = ['a.pay_status','=',1];
    ...
    public function fenpei(){
       $info =  $this->model->where(['id'=>$this->in['id']])->find();
        $info = $this->model->handle($info);
...
//           master_id
            ...
            if($this->in['master_id']){
                $data = [];
                $data['order_id'] = $this->in['id'];
                $data['uid'] = $this->in['master_id'];
                $data['order_status'] = 2;
                $this->model->updateOrderStatus($data);
            }
    ...
    public function zhifu(){
        $info = $this->model->where(['id'=>$this->in['id']])->find();
        $res = $this->model->pay_success($info);
        $this->success($res['info']);
    }
...
//        $where[] = ['a.pay_status','=',1];
        ...
        foreach ($data as &$v) {
            ...
            $v['address'] = $v['province'].$v['city'] .$v['country'] .$v['reference'].$v['address'];
            unset($v['reference']);
            if (isset($v['user_id'])) {
