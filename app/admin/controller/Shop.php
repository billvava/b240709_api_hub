    ...
    */
    public function index() {
        ...
        $data['list'] = Db::name($this->db_name)
                ...
                ->order($order)
                ->select()->toArray();
        $array = [4=>'店家',3=>'县级代理',2=>'市级代理'];
        $state_array = ['待审核','审核通过','已拒绝'];
        foreach ($data['list'] as $key => &$value){
           $province = Db::name('system_areas') -> where('id',$value['province']) ->value('name');
           $city = Db::name('system_areas') -> where('id',$value['city']) ->value('name');
           $country = Db::name('system_areas') -> where('id',$value['country']) ->value('name');
           $value['address'] = $province.$city.$country.$value['address'];
           $value['role_name'] = $array[$value['type']]??'';
           $value['state_text'] = $state_array[$value['state']]??'';
        }
    ...
    public function chuli(){
        $info =  Db::name($this->db_name)->where([
            ['state','=',0],
            ['id','=',$this->in['id']],
        ...
        if($this->in['state'] == 1 ){
            $uinfo =  (new User())->getUserInfo($info['uid']);
            if($uinfo['pid']){

                $agent_bonus = C('agent_bonus');
                $agent_bonus_array = explode('|',$agent_bonus);
                $bonus = $agent_bonus_array[$info['type']-2];

                 if($bonus > 0){
                    $umodel = (new User());
                    $pid = $uinfo['pid'];

                    $umodel->handleUser('money',$pid,$bonus,1,['ordernum'=>'','cate'=>14]);
                  
                 }
            }
             (new User())->where(['id'=>$info['uid']])->update([
                        'role'=>$info['type']
                    ]);
