...
<div class='x-body' >
    ...
    <?php if($is_add==1){ ?>
    <xblock>
        <a class="pear-btn pear-btn-primary " href="{:url('add',['type'=>2])}"> <i class="layui-icon layui-icon-add-1"></i> 新增</a>
    </xblock>
    <?php } ?>
    <?php if($is_search==1){ ?>
...
<?php $itms =  $all_lan['status']; ?><?php  echo new_list_select(array('field'=>'status','items'=>$itms,'fname'=>'状态','defaultvalue'=>input('get.status'),'requestFnName'=>'status')); ?>
            ...
            <?php $itms =  $shop_list; ?><?php  echo new_list_select(array('field'=>'id','items'=>$itms,'fname'=>'门店','defaultvalue'=>input('get.id'),'requestFnName'=>'id')); ?>
<div class='layui-inline' >
		...
		<input type='text' name='linkman'  value='<?php echo input('get.linkman'); ?>' class='layui-input'>
            ...
            <div class="layui-form-item" style=" margin-top: 10px;">
                <div class='layui-inline'>
                    ...
                    <div class='layui-input-inline'>
                        <button class="pear-btn pear-btn-primary  "  type="button"  onclick="sub('{:url('index2')}')" >搜索</button>
                        <?php if($is_xls==1){ ?>
