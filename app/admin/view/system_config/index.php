
<div class='x-body' >


     <?php if($is_add==1){ ?>     
    <xblock>
         <a class="pear-btn pear-btn-primary " href="{:url('add')}"> <i class="layui-icon layui-icon-add-1"></i> 新增</a>
    </xblock>
     <?php } ?>        
     <?php if($is_search==1){ ?>  
      <div class="layui-row mt10">
        <form class="layui-form layui-col-md12 x-so" id="index_form" action=""  method="get">
<input type='text' name='name' value='<?php echo input('get.name'); ?>'  placeholder='中文名称'  class='layui-input'>
<input type='text' name='field' value='<?php echo input('get.field'); ?>'  placeholder='字段'  class='layui-input'>

<select name='is_del' lay-ignore  class='inline w150'>
                    <option value=''>可删除</option>
                    <?php foreach($all_lan['is_del'] as $k=>$v){ ?>
                    <option value='{$k}' <?php if($in['is_del']==$k && $in['status']!='' ){ echo 'selected=""'; } ?> >{$v}</option>
                    <?php } ?>
                </select>
<select name='is_show' lay-ignore  class='inline w150'>
                    <option value=''>显示状态</option>
                    <?php foreach($all_lan['is_show'] as $k=>$v){ ?>
                    <option value='{$k}' <?php if($in['is_show']==$k && $in['status']!='' ){ echo 'selected=""'; } ?> >{$v}</option>
                    <?php } ?>
                </select>

<select name='cate_id' lay-ignore  class='inline w150'>
                    <option value=''>分组</option>
                    <?php foreach($cate as $k=>$v){ ?>
                    <option value='{$v['id']}' <?php if($in['cate_id']==$v['id'] && $in['cate_id']!='' ){ echo 'selected=""'; } ?> >{$v.name_pre}{$v.name}</option>
                    <?php } ?>
</select>


            <input type="hidden"  name="p" value="1">
            <button class="pear-btn pear-btn-primary  "  type="button"  onclick="sub('{:url('index')}')" >搜索</button>
            <?php if($is_xls==1){ ?>  
            <button class="pear-btn pear-btn-primary  "  type="button"  onclick="sub('{:url('xls')}')">导出</button>
            <?php } ?>
        </form>
    </div>
            
    
     <?php } ?>    
    
    <div class="table-container">
        <table class="layui-table" >
   
    <thead>
        <tr>
            <?php if($is_del==1){ ?>
            <th>
                 <a  class="x-a" href="javascript://" onclick="$('input[type=checkbox]').prop('checked',true)" >全选</a> | 
                <a class="x-a"  href="javascript://" onclick="$('input[type=checkbox]').each(function(){var t = $(this); t.prop('checked',!t.is(':checked')); })">反选</a>
             </th>
              <?php } ?>
             <th>字段名</th>
<th>中文名称</th>
<th>提示</th>
<th>值</th>
<th>可删除</th>
<th>显示状态</th>
<th>单位</th>
<th>分组ID</th>
<th>排序</th>

            <th>操作</th>
        </tr>
    </thead>
    <tbody>
        {foreach name='data.list' item='v' key='k' }
          <tr class="layui-form">
                <?php if($is_del==1){ ?><th style=" width: 100px;"><input  class="sel_id"  type="checkbox" lay-ignore value="<?php echo $v[$pk]; ?>" name="<?php echo $pk; ?>[]"/></th> <?php } ?>
              <th><?php echo $v['field']; ?></th>
<th><?php echo fast_input(array(
                      'key'=>$pk,
                       'keyid'=>$v[$pk],
                       'field'=>'name',
                        'val'=> $v['name']
                  )); ?></th>
<th style="width: 50px;"><?php echo $v['msg']; ?></th>
<th  style="width: 50px;"><?php echo $v['val']; ?></th>
<th><?php echo $all_lan['is_del'][$v['is_del']]; ?></th>
<th>
    <?php echo fast_check(array(
        'key'=>'id',
        'keyid'=>$v['id'],
        'check'=>$v['is_show'],
        'txt'=>'显示',
        'url'=>url('set_val'),
        'field'=>'is_show'
    )); ?>
</th>
<th><?php echo fast_input(array(
                      'key'=>$pk,
                       'keyid'=>$v[$pk],
                       'field'=>'unit',
                        'val'=> $v['unit']
                  )); ?></th>
<th>
    <select lay-ignore  data-type='setval'  data-key="id" data-keyid="{$v.id}" data-url="{:url('set_val')}" data-field="cate_id" >
        <option value="">分组</option>
          <?php foreach($cate as $ck=>$cv){ ?>
            <option value='{$cv.id}' <?php if($v['cate_id']==$cv['id'] && $v['cate_id']!='' ){ echo 'selected=""'; } ?> >{$cv.name_pre}{$cv.name}</option>
            <?php } ?>
    </select>
</th>
<th><?php echo fast_input(array(
                      'key'=>$pk,
                       'keyid'=>$v[$pk],
                       'field'=>'sort',
                        'val'=> $v['sort']
                  )); ?></th>

                <th>
                     <?php if($is_edit==1){ ?>
                    <a  class="pear-btn pear-btn-primary pear-btn-sm" href="<?php echo url('edit',array($pk=>$v[$pk]) ); ?>">编辑</a>
                     <?php } ?>
                      <?php if($is_del==1){ ?>
                    <a class="pear-btn pear-btn-danger pear-btn-sm" href="<?php echo url('delete',array($pk=>$v[$pk]) ); ?>" onclick="return window.confirm('此操作不可撤销，你确定要继续？');">删除</a>
                     <?php } ?>
                </th>
            </tr>     
         {/foreach}
         <?php if($is_del==1){ ?>
         <tr>
             <td colspan="100">
                 <a  href="javascript://" onclick="forever_copy()"   >复制</a> | <a  href="javascript://" onclick="forever_del()"   >批量删除</a>
             </td>
         </tr>
         <?php } ?>
    </tbody>
</table>
              {$data.page|raw}
    </div>          
</div>
<script type="text/javascript">
function sub(url){
    $("#index_form").attr('action',url).submit();
}    
function all_hand(field,val){
    var ids = get_ids();
    if(ids==null){
        layer.alert('请先选择');
        return;
    }
    ajax('{:url('mul_set')}',{ '<?php echo $pk; ?>':ids ,field:field,val:val} );
}
function get_ids(){
    var ids = [];
    $(".sel_id:checked").each(function() {
        ids.push($(this).val());
    });
    if(ids.length <= 0){
        
        return null;
    }else{
        return ids;
    }
}

function forever_copy(){
    var ids = get_ids();
    if(ids==null){
        layer.alert('请先选择');
        return;
    }
    var r=window.confirm("确定复制吗？");
    if(r){
        ajax('{:url('copy')}',{"<?php echo $pk; ?>":ids } );
    }
    
}
function forever_del(){
    var ids = get_ids();
    if(ids==null){
        layer.alert('请先选择');
        return;
    }
    var r=window.confirm("确定删除吗？");
    if(r){
        ajax('{:url('delete')}',{"<?php echo $pk; ?>":ids } );
    }
}
</script>
