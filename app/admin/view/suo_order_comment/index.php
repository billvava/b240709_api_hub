...
<div class='x-body' >
    ...
    <div class="layui-row mt10">
        <form class="layui-form layui-col-md12 x-so" id="index_form" action=""  method="get">
            <input type='text' name='order_id'  value='<?php echo input('get.order_id'); ?>' class='layui-input'>
...
<th>状态</th>
            ...
            {foreach name='data.list' item='v' key='k' }
                <tr class="layui-form">
                    ...
                    <th><?php echo $v['id']; ?></th>
                    <th><?php echo getname($v['user_id']); ?>
                        <?php if($v['order_id']){ ?>
                            <p><a href="javascript://" onclick="show_url('<?php echo url('SuoOrder/index',['id'=>$v['order_id']]); ?>')">相关订单</a></p>
                        <?php } ?>
                    </th>
                    <th><?php $minfo =  (new \app\admin\model\SuoMaster())->getInfo($v['master_id'],true) ; echo $minfo['realname']; ?></th>

                    <th><?php echo $v['content']; ?></th>
                    <th><?php echo $v['reply']; ?></th>


                    <th><?php  echo_img($v['images']); ?></th>
