 ...
 * @mixin think\Model
 */
class UserGongxianzhi extends Model {


    protected $name='user_gongxianzhi';
    ...
    public  function getAll($where = array(),$cache=true){
       ...
       $name = "user_gongxianzhi_1721377786_{$pre}";
       $data = cache($name);
       if(!$data  || !$cache){
           $order = "id desc";
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "user_gongxianzhi_info_1721377786_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function clear($pk = ''){
        $name = "user_gongxianzhi_1721377786";
        cache($name,null);
        if ($pk) {
            $name = "user_gongxianzhi_info_1721377786_{$pk}";
            cache($name, null);
