 ...
 * @mixin think\Model
 */
class UserLvseDot extends Model {


    protected $name='user_lvse_dot';
    ...
    public  function getAll($where = array(),$cache=true){
       ...
       $name = "user_lvse_dot_1721476437_{$pre}";
       $data = cache($name);
       if(!$data  || !$cache){
           $order = "id desc";
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "user_lvse_dot_info_1721476437_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
