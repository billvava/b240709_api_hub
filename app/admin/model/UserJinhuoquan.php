 ...
 * @mixin think\Model
 */
class UserJinhuoquan extends Model {


    protected $name='user_jinhuoquan';
    ...
    public  function getAll($where = array(),$cache=true){
       ...
       $name = "user_jinhuoquan_1721377934_{$pre}";
       $data = cache($name);
       if(!$data  || !$cache){
           $order = "id desc";
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "user_jinhuoquan_info_1721377934_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function clear($pk = ''){
        $name = "user_jinhuoquan_1721377934";
        cache($name,null);
        if ($pk) {
            $name = "user_jinhuoquan_info_1721377934_{$pk}";
            cache($name, null);
