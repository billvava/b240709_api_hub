...
class SystemLog extends Model {
    ...
    public  function getAll($where = array()){
       ...
       $name = "system_log_1640743333_{$pre}";
       $data = cache($name);
       if(!$data){
           $order = "id desc";
    ...
    public  function getInfo($pk) {
        ...
        $name = "system_log_info_1640743333_{$pk}";
        $data = cache($name);
        if (!$data) {
            $mypk = $this->get_pk();
            if (!$mypk) {
...
'param'=>'操作参数',
    ...
    public  function rules() {
        return [
            'rule'=>[
            'node|操作结点'=>["require","max"=>255,],
'create_time|操作时间'=>["require",],
'admin_id|操作人'=>["require","integer",],
'name|操作名称'=>["require","max"=>64,],
'param|操作参数'=>["require",],
