 ...
 * @mixin think\Model
 */
class SuoOrderlog extends Model {


    protected $name='suo_orderlog';
    ...
    public  function getAll($where = array(),$cache=true){
       ...
       $name = "suo_orderlog_1675933242_{$pre}";
       $data = cache($name);
       if(!$data  || !$cache){
           $order = "id desc";
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "suo_orderlog_info_1675933242_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
