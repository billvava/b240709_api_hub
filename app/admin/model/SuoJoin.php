 ...
 * @mixin think\Model
 */
class SuoJoin extends Model {


    protected $name='suo_join';
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "suo_join_info_1677203842_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
...
'create_time'=>'创建时间',
    ...
    public  function rules() {
        return [
            'rule'=>[
            'realname|真实姓名'=>["max"=>20,],
'tel|tel'=>["max"=>15,],
'activity_id|活动id'=>["integer",],
'activity_name|活动名称'=>["max"=>255,],
'create_time|创建时间'=>[],
