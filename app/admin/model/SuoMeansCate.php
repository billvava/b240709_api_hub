 ...
 * @mixin think\Model
 */
class SuoMeansCate extends Model {


    protected $name='suo_means_cate';
    ...
    public  function getAll($where = array(),$cache=true){
       ...
       $name = "suo_means_cate_1677119383_{$pre}";
       $data = cache($name);
       if(!$data  || !$cache){
           $order = "sort asc,id desc";
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "suo_means_cate_info_1677119383_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function clear($pk = ''){
        $name = "suo_means_cate_1677119383";
        cache($name,null);
        if ($pk) {
            $name = "suo_means_cate_info_1677119383_{$pk}";
            cache($name, null);
