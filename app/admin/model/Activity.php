...
class Activity extends Model {
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "activity_info_1721895933_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public function getLan($field=''){
        $lans = array('status' => array('0'=>'未开奖','1'=>'已开奖',),);
        if($field==''){
            return $lans;
        }
        return $lans[$field];
...
'status'=>'状态',
    ...
    public  function rules() {
        return [
            'rule'=>[
            'strat_time|开始时间'=>["integer",],
'end_time|结束时间'=>["integer",],
'create_time|时间'=>[],
'winning_numbers|中奖号码'=>["integer",],
'numbers|累计人数'=>["integer",],
'count_daijinquan|累计代金券'=>["float",],
'status|状态'=>["integer",],
