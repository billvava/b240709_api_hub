 ...
 * @mixin think\Model
 */
class SuoShop extends Model {


    protected $name='suo_shop';
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "suo_shop_info_1676534553_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function getOption($name = 'name',$w=[]) {
        $as = $this->getAll($w);
        $this->open_name=$name;
