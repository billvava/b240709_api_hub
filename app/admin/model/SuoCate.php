 ...
 * @mixin think\Model
 */
class SuoCate extends Model {


    protected $name='suo_cate';
    ...
    public  function getInfo($pk,$cache=false) {
        ...
        $name = "suo_cate_info_1675933220_{$pk}";
        $data = cache($name);
        if (!$data || !$cache) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function getOption($name = 'name',$where = []) {
        $as = $this->getAll($where);
        $this->open_name=$name;
