<div class="x-body">
    ...
    $vals = array();  
    $ts = array();
  $data = $data?$data:[];  foreach($data as $k=>$v){ 
        $vals[] = $v+0; 
        $names[] = "'".$k."号'";
        if($total_field){
            $ts[] = $totals[$k]+0;
    ...
    <div class="mt10">
        <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="<?php echo url('day'); ?>">
            <input type="text" value="{$time}" readonly="" name="time"    class="layui-input jeYearMouth">
            <button class="layui-btn layui-btn-sm"  type="submit" ><i class="layui-icon">&#xe615;</i></button>
        </form>
    </div>
