...
class MsGoods extends BCOM{
    ...
    public function __construct(App $app)
    {
        ...
        $this->model = new \app\mall\model\MsGoods();
        $this->db_name=$this->model->dbName();
        $cate = (new \app\mall\model\MsCate())->getOption();
    ...
    */
    ...
    private function item() {
        ...
        if ($this->request->isPost()) {
            ...
            $in['min_price'] = min(array_filter($in['goods_id_items']));
            $in['items'] = json_encode(array_filter($in['goods_id_items']));
            if ($in['min_price'] <= 0.01) {
                $this->error('价格最低0.01');
            }
//           $category_id =  Db::name('mall_goods')->where(['goods_id'=>$in['goods_id']])->value('category_id');
//            $in['type'] = (new \app\mall\model\GoodsCategory())->getDingji($category_id);
