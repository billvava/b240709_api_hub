...
class Data {
    ...
    public function getBuy($in) {
        ...
        $count = Db::name('user_ext')->where($where)->alias('a')->join('user b','a.user_id=b.id')->count();
        tool()->classs('PageForAdmin');
        $Page = new \PageForAdmin($count, $page_num);
        $start = $Page->firstRow;

        $data['list'] = Db::name('user_ext')->alias('a')->join('user b','a.user_id=b.id')->field("a.*,b.username,b.rank")->where($where)->limit($start ,$page_num+0)->select()->toArray();
        $data['page'] = $Page->show();
        $data['count'] = $count;
    ...
    public function user_rank($in) {
        ...
        $data['list'] = Db::name('mall_order')->where($where)
                ->field(array('user_id', 'username', "count(*) as count", 'sum(goods_total) as total'))
