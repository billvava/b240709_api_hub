...
class ExpressCompany extends Model {
    ...
    protected $name='mall_express_company';
    ...
    public  function getAll($where = array()){
       ...
       $name = "mall_express_company_1616139628_{$pre}";
       $data = cache($name);
       if(!$data){
           $order = "id desc";
    ...
    public  function getInfo($pk) {
        ...
        $name = "mall_express_company_info_1616139628_{$pk}";
        $data = cache($name);
        if (!$data) {
            $mypk = $this->get_pk();
            if (!$mypk) {
    ...
    public  function clear($pk = ''){
        $name = "mall_express_company_1616139628";
        cache($name,null);
        if ($pk) {
            $name = "mall_express_company_info_1616139628_{$pk}";
            cache($name, null);
...
'expresstelephone'=>'快递电话',
    ...
    public  function rules() {
        return [
            'rule'=>[
            'status|是否显示'=>["integer",],
'expresskey|快递code'=>["max"=>55,],
'expressname|快递名称'=>["max"=>55,],
'expresswebsite|快递官网'=>["max"=>255,],
'expresstelephone|快递电话'=>["max"=>25,],
