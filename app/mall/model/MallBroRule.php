 ...
 * @mixin think\Model
 */
class MallBroRule extends Model {


    protected $name='mall_bro_rule';
    ...
    public  function getAll($where = array()){
       ...
       $name = "mall_bro_rule_1616059727_{$pre}";
       $data = cache($name);
       if(!$data){
           $order = "id desc";
    ...
    public  function getInfo($pk) {
        ...
        $name = "mall_bro_rule_info_1616059727_{$pk}";
        $data = cache($name);
        if (!$data) {
            $mypk = $this->get_pk();
            if (!$mypk) {
...
'bro3'=>'三级佣金比例',
    ...
    public  function rules() {
        return [
            'rule'=>[
            'name|规则名称'=>['require',"max"=>55,],
'bro1|一级佣金比例'=>["float","egt"=>0,],
'bro2|二级佣金比例'=>["float",],
'bro3|三级佣金比例'=>["float","egt"=>0,],
