...
class MallCoupon extends Model {
    ...
    public function send_tpl($user_id, $tpl_id, $num = 1, $source = 1) {
        if ($num <= 0) {
    ...
    //可用数量
    public function getCanCount($user_id) {
        $where = array(
            ['user_id','=',$user_id],
            ['status','=',0],
    ...
    public function send($map = array()) {
        /*
          name  base_money money start end time user_id  status
         *          */
        return $this->insertGetId(array(
            ...
            'time' => date('Y-m-d H:i:s'),
            'user_id' => $map['user_id'] ? $map['user_id'] : 0,
            'status' => 0,
