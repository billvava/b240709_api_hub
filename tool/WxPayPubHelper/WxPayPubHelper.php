...
class Common_util_pub {
    ...
    public $appid = null; //微信公众号身份的唯一标识
    public $mchid = null; //商户号ID
    public $key = null;  //商户支付密钥Key
    public $appsecret = null; //app秘钥
    public $timeout = 30; //curl超时时间
    ...
    function __construct($config = array()) {
        ...
        $this->appid = $conf['appid'] ? $conf['appid'] : C('appid');
        $this->mchid = $conf['wxshopid'] ? $conf['wxshopid'] : C('wxshopid');
        $this->key = $conf['wxshoppwd'] ? $conf['wxshoppwd'] : C('wxshoppwd');
        $this->appsecret = $conf['apppwd'] ? $conf['apppwd'] : C('apppwd');
        if (!$this->appid) {
            throw new SDKRuntimeException("微信应用ID不能为空！");
        }
        if (!$this->mchid) {
            throw new SDKRuntimeException("微信商户ID不能为空！");
        }
        if (!$this->key) {
            throw new SDKRuntimeException("微信支付秘钥不能为空！");
        }
        if (!$this->appsecret) {
            throw new SDKRuntimeException("微信应用秘钥不能为空！");
        }
        $this->SSLCERT_PATH = getcwd() . "/tool/WxPayPubHelper/cacert/apiclient_cert.pem";
        $this->SSLKEY_PATH = getcwd() . "/tool/WxPayPubHelper/cacert/apiclient_key.pem";
    }
...
//        curl_close($ch);
        ...
        if (function_exists('simplexml_load_string')) {
            ...
            if ($res->return_code != 'SUCCESS') {
                echo ("<h1>微信发起失败：{$res->return_msg}</h1>" );
                die;
            }
        }
        //返回结果
...
class UnifiedOrder_pub extends Wxpay_client_pub {
    ...
    function __construct($conf = array()) {
        ...
        //设置curl超时时间
        $this->curl_timeout = $this->timeout;
    }
